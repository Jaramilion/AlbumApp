  ...
  reducers: {
    ...
    getAlbumsDataSuccess: (state, action: PayloadAction<AlbumData>) => {
      return {
        ...
        albumDataStatus: STATUSES.SUCCESS,
      };
    },
    ...
    deleteAlbumFromUser: (
      ...
      const userIndex = state.albumData.findIndex(
        u => u.id === action.payload.userId,
      );
      const userAlbumIndex = state.albumData[userIndex].data.findIndex(
        a => a.id === action.payload.albumId,
      );
    ...
    getPhotosByAlbumSuccess: (state, action: PayloadAction<PhotosByAlbum>) => {
      return {
        ...state,
        photosByAlbum: [...action.payload],
        photosByAlbumStatus: STATUSES.SUCCESS,
      };
    },
